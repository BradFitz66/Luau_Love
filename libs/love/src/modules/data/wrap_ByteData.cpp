...
int w_ByteData_setString(lua_State *L)
{
	...
	if (offset < 0 || offset + (int64) size > (int64) t->getSize())
		return luaX_error(L, "The given string offset and size don't fit within the Data's size.");
...
static int w_ByteData_setT(lua_State *L)
{
	...
	if (offset < 0 || offset + sizeof(T) * nargs > t->getSize())
		return luaX_error(L, "The given offset and value parameters don't fit within the Data's size.");
