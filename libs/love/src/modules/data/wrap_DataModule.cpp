...
int w_newDataView(lua_State *L)
{
	...
	if (offset < 0 || size < 0)
		return love::luaX_error(L, "DataView offset and size must not be negative.");
...
int w_newByteData(lua_State *L)
{
	...
	if (luax_istype(L, 1, Data::type))
	{
		...
		if (data->getSize() > std::numeric_limits<lua_Integer>::max())
			return love::luaX_error(L, "Data's size is too large!");

		lua_Integer offset = luaL_optinteger(L, 2, 0);
		if (offset < 0)
			return love::luaX_error(L, "Offset argument must not be negative.");

		lua_Integer size = luaL_optinteger(L, 3, data->getSize() - offset);
		if (size <= 0)
			return love::luaX_error(L, "Size argument must be greater than zero.");
		else if ((size_t)(offset + size) > data->getSize())
			return love::luaX_error(L, "Offset and size arguments must fit within the given Data's size.");
	...
	else
	{
		...
		if (size <= 0)
			return love::luaX_error(L, "Data size must be a positive number.");
...
int w_pack(lua_State *L)
{
	if (luax_istype(L, 1, ByteData::type))
	{
		...
		if (offset + b.nelems > d->getSize())
		{
			...
			return love::luaX_error(L, "The given byte offset and pack format parameters do not fit within the ByteData's size.");
		}
