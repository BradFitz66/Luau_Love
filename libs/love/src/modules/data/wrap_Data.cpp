...
int w_Data_getString(lua_State *L)
{
	...
	if (size <= 0)
		return luaX_error(L, "Invalid size parameter (must be greater than 0)");

	if (offset < 0 || offset + size > (int64) t->getSize())
		return luaX_error(L, "The given offset and size parameters don't fit within the Data's size.");
...
static int w_Data_getT(lua_State* L)
{
	...
	if (count <= 0)
		return luaX_error(L, "Invalid count parameter (must be greater than 0)");

	if (offset < 0 || offset + sizeof(T) * count > t->getSize())
		return luaX_error(L, "The given offset and count parameters don't fit within the Data's size.");
