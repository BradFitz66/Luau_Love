...
static int w_Shader_sendLuaValues(lua_State *L, int startidx, Shader *shader, const Shader::UniformInfo *info, const char *name)
{
	...
	default:
		return luaX_error(L, "Unknown variable type for shader uniform '%s", name);
	}
}
...
static int w_Shader_sendData(lua_State *L, int startidx, Shader *shader, const Shader::UniformInfo *info, bool colors)
{
	if (info->baseType == Shader::UNIFORM_SAMPLER || info->baseType == Shader::UNIFORM_STORAGETEXTURE
		...
		return luaX_error(L, "Only value types (floats, ints, vectors, matrices, etc) be sent to Shaders via Data objects.");
	...
	if (offset < 0)
		return luaX_error(L, "Offset cannot be negative.");
	else if ((size_t) offset >= size)
		return luaX_error(L, "Offset must be less than the size of the Data.");
	...
	if (!lua_isnoneornil(L, startidx + 2))
	{
		...
		if (sizearg <= 0)
			return luaX_error(L, "Size must be greater than 0.");
		else if ((size_t) sizearg > size - offset)
			return luaX_error(L, "Size and offset must fit within the Data's bounds.");
		else if (sizearg % uniformstride != 0)
			return luaX_error(L, "Size (%d) must be a multiple of the uniform's size in bytes (%d).", sizearg, uniformstride);
		else if ((size_t) sizearg > info->dataSize)
			return luaX_error(L, "Size must not be greater than the uniform's total size in bytes.");
	...
	if (size == 0)
		return luaX_error(L, "Size to copy must be greater than 0.");
...
int w_Shader_sendColors(lua_State *L)
{
	...
	if (info->baseType != Shader::UNIFORM_FLOAT || info->components < 3)
		return luaX_error(L, "Shader:sendColor can only be used with vec3 or vec4 uniforms.");
...
int w_Shader_getBufferFormat(lua_State *L)
{
	...
	return luaX_error(L, "Buffer '%s' does not exist in the Shader.", name);
}
