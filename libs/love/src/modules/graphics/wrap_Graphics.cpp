...
static int luax_checkgraphicscreated(lua_State *L)
{
	if (!instance()->isCreated())
		return luaX_error(L, "love.graphics cannot function without a window.");
	return 0;
}
...
int w_setCanvas(lua_State *L)
{
		...
		for (int i = 1; i <= (int) luax_objlen(L, 1); i++)
		{
			...
			else
			{
				...
				if (targets.colors.back().texture->getTextureType() != TEXTURE_2D)
					return luaX_error(L, "Non-2D textures must use the table-of-tables variant of setCanvas.");
			}
	...
	else
	{
		for (int i = 1; i <= lua_gettop(L); i++)
		{
			...
			if (i > 1 && type != TEXTURE_2D)
				return luaX_error(L, "This variant of setCanvas only supports 2D texture types.");
...
static void screenshotFunctionCallback(const Graphics::ScreenshotInfo *info, love::image::ImageData *i, void *gd)
{
	...
	{
		if (ref == nullptr)
			luaX_error(L, "Internal error in screenshot callback.");
...
int w_setScissor(lua_State *L)
{
	...
	if (rect.w < 0 || rect.h < 0)
		return luaX_error(L, "Can't set scissor with negative width and/or height.");
...
int w_getStencilMode(lua_State *L)
{
	...
	if (!getConstant(mode, modestr))
		return luaX_error(L, "Unknown stencil mode.");
...
int w_getStencilState(lua_State *L)
{
	...
	if (!getConstant(s.action, actionstr))
		return luaX_error(L, "Unknown stencil draw action.");

	const char *comparestr;
	if (!getConstant(s.compare, comparestr))
		return luaX_error(L, "Unknown compare mode.");
...
getImageData(lua_State *L, int idx, bool allowcompressed, float *dpiscale)
{
	...
	else if (filesystem::luax_cangetdata(L, idx))
	{
		...
		if (imagemodule == nullptr)
			luaX_error(L, "Cannot load images without the love.image module.");
...
static void luax_checktexturesettings(lua_State *L, int idx, bool opt, bool checkType, bool checkDimensions, OptionalBool forceRenderTarget, Texture::Settings &s, bool &setdpiscale)
{
	...
	if (!lua_isnoneornil(L, -1))
	{
		if (lua_type(L, -1) != LUA_TTABLE)
			luaX_argerror(L, idx, "expected field 'viewformats' to be a table type");
...
int w_newCubeTexture(lua_State *L)
{
	...
	else
	{
		...
		else
		{
			...
			{
				if (tlen != 6)
					return luaX_error(L, "Cubemap images must have 6 faces.");
...
int w_newParticleSystem(lua_State *L)
{
	...
	if (size < 1.0 || size > ParticleSystem::MAX_PARTICLES)
		return luaX_error(L, "Invalid ParticleSystem size");
...
static int w_getShaderSource(lua_State *L, int startidx, std::vector<std::string> &stages, Shader::CompileOptions &options)
{
		...
		else
		{
			...
			if (slen > 0 && slen < 64 && !strchr(str, '\n'))
			{
				...
				if (ext != nullptr && !strchr(ext, ';') && !strchr(ext, ' '))
					return luaX_error(L, "Could not open file %s. Does not exist.", str);
			}
		}
	}
	...
	if (!lua_isnoneornil(L, optionsidx))
	{
		...
		{
			if (!lua_istable(L, -1))
				luaX_argerror(L, optionsidx, "expected 'defines' field to be a table");
			...
			while (lua_next(L, -2))
			{
				...
				if (lua_type(L, -2) == LUA_TNUMBER && lua_type(L, -1) == LUA_TSTRING)
					...
					luaX_argerror(L, optionsidx, "all fields in the 'defines' table must use string keys.");
				else
				{
					...
					else
					{
						...
						if (val == nullptr)
							luaX_argerror(L, optionsidx, "'defines' table values must be strings, numbers, or booleans.");
						defval = val;
					}
				}
...
static Buffer::DataDeclaration luax_checkdatadeclaration(lua_State* L, int formattableidx, int arrayindex, int declindex, bool requirename, bool requirelocation)
{
	...
	if (requirename && lua_type(L, -1) != LUA_TSTRING)
	{
		...
		luaX_argerror(L, formattableidx, str.c_str());
	}
...
static Buffer *luax_newbuffer(lua_State *L, int idx, Buffer::Settings settings, const std::vector<Buffer::DataDeclaration> &format)
{
	...
	if (lua_istable(L, idx))
	{
		...
		if (!tableoftables)
		{
			if (arraylength % ncomponents != 0)
				luaX_error(L, "Array length in flat array variant of newBuffer must be a multiple of the total number of components (%d)", ncomponents);
	...
	else if (data == nullptr)
	{
		...
		if (len <= 0)
			luaX_argerror(L, idx, "number of elements must be greater than 0");
...
static Mesh *newCustomMesh(lua_State *L)
{
	...
	if (!lua_istable(L, -1))
	{
		luaX_argerror(L, 1, "table of tables expected");
		return nullptr;
	}
	lua_pop(L, 1);
		...
		else
		{
			...
			if (strcmp(tname, "byte") == 0 || strcmp(tname, "unorm8") == 0)
			{
				if (components == 4)
					...
					luaX_error(L, "Invalid component count (%d) for vertex data type %s", components, tname);
			}
	...
	else
	{
		...
		if (!lua_istable(L, -1))
		{
			luaX_argerror(L, 2, "expected table of tables");
			return nullptr;
		}
		lua_pop(L, 1);
...
int w_newMesh(lua_State *L)
{
	...
	if (arg1type != LUA_TTABLE && arg1type != LUA_TNUMBER)
		luaX_argerror(L, 1, "table or number expected");
...
int w_getBlendMode(lua_State *L)
{
	...
	if (!getConstant(mode, str))
		return luaX_error(L, "Unknown blend mode");

	if (!getConstant(alphamode, alphastr))
		return luaX_error(L, "Unknown blend alpha mode");
...
static void luax_pushblendop(lua_State *L, BlendOperation op)
{
	...
	if (!getConstant(op, str))
		luaX_error(L, "unknown blend operation");
	lua_pushstring(L, str);
}
...
static void luax_pushblendfactor(lua_State *L, BlendFactor factor)
{
	...
	if (!getConstant(factor, str))
		luaX_error(L, "unknown blend factor");
	lua_pushstring(L, str);
}
...
int w_getDefaultFilter(lua_State *L)
{
	...
	if (!SamplerState::getConstant(s.minFilter, minstr))
		return luaX_error(L, "Unknown minification filter mode");
	if (!SamplerState::getConstant(s.magFilter, magstr))
		return luaX_error(L, "Unknown magnification filter mode");
...
int w_getLineStyle(lua_State *L)
{
	...
	if (!Graphics::getConstant(style, str))
		return luaX_error(L, "Unknown line style");
	lua_pushstring(L, str);
	return 1;
}
...
int w_getLineJoin(lua_State *L)
{
	...
	if (!Graphics::getConstant(join, str))
		return luaX_error(L, "Unknown line join");
	lua_pushstring(L, str);
	return 1;
}
...
int w_getMeshCullMode(lua_State *L)
{
	...
	const char *str;
	if (!getConstant(mode, str))
		return luaX_error(L, "Unknown cull mode");
	lua_pushstring(L, str);
	return 1;
}
...
int w_getFrontFaceWinding(lua_State *L)
{
	...
	if (!getConstant(winding, str))
		return luaX_error(L, "Unknown vertex winding");
	lua_pushstring(L, str);
	return 1;
}
...
int w_points(lua_State *L)
{
	...
	if (args % 2 != 0 && !is_table_of_tables)
		return luaX_error(L, "Number of vertex components must be a multiple of two");
...
int w_line(lua_State *L)
{
	...
	if (arg1type != LUA_TTABLE && arg1type != LUA_TNUMBER)
		...
		return luaX_error(L, "Need at least two vertices to draw a line.");
...
int w_polygon(lua_State *L)
{
	...
	if (args % 2 != 0)
		...
		return luaX_error(L, "Need at least three vertices to draw a polygon");
...
int w_copyBuffer(lua_State *L)
{
	...
	if (sourceoffset < 0 || destoffset < 0)
		return luaX_error(L, "copyBuffer offsets cannot be negative.");
	if (size <= 0)
		return luaX_error(L, "copyBuffer size must be greater than 0.");
...
int w_copyBufferToTexture(lua_State *L)
{
	...
	ptrdiff_t sourceoffset = luaL_optinteger(L, 3, 0);
	if (sourceoffset < 0)
		return luaX_error(L, "copyBufferToTexture source offset cannot be negative.");
...
int w_copyTextureToBuffer(lua_State *L)
{
	...
	if (destoffset < 0)
		return luaX_error(L, "copyTextureToBuffer dest offset cannot be negative.");
...
extern "C" int luaopen_love_graphics(lua_State *L)
{
	...
	if (instance == nullptr)
	{
		...
		return luaX_error(L, "Cannot create graphics: no supported renderer on this system.");
	}
	...
	if (luax_loadbufferx(L, (const char *)graphics_lua, sizeof(graphics_lua), "=[love \"wrap_Graphics.lua\"]") == 0)
		lua_call(L, 0, 0);
	else
		lua_error(L);
