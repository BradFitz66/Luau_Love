...
static int w_Buffer_setArrayData(lua_State *L)
{
	...
	if (sourceindex < 0)
		return luaX_error(L, "Source start index must be at least 1.");
	...
	if (!lua_isnoneornil(L, 5))
	{
		...
		if (count <= 0)
			return luaX_error(L, "Element count must be greater than 0.");
	}
	...
	if (destindex >= arraylength || destindex < 0)
		return luaX_error(L, "Invalid buffer start index (must be between 1 and %d)", arraylength);
	...
	if (luax_istype(L, 2, Data::type))
	{
		...
		if (sourceindex >= dataarraylength)
			return luaX_error(L, "Invalid data start index (must be between 1 and %d)", dataarraylength);
		...
		if (count > maxcount)
			return luaX_error(L, "Too many array elements (expected at most %d, got %d)", maxcount, count);
	...
	if (!tableoftables)
	{
		if (tablelen % ncomponents != 0)
			return luaX_error(L, "Array length in flat array variant of Buffer:setArrayData must be a multiple of the total number of components (%d)", ncomponents);
		tablelen /= ncomponents;
	}

	if (sourceindex >= tablelen)
		return luaX_error(L, "Invalid data start index (must be between 1 and %d)", tablelen);

	count = count >= 0 ? std::min(count, tablelen - sourceindex) : tablelen - sourceindex;

	if (destindex + count > arraylength)
		return luaX_error(L, "Too many array elements (expected at most %d, got %d)", arraylength - destindex, count);
...
static int w_Buffer_clear(lua_State *L)
{
	...
	if (!lua_isnoneornil(L, 2))
	{
		...
		if (offsetp < 0 || sizep < 0)
			return luaX_error(L, "Offset and size parameters cannot be negative.");
