...
int w_Mesh_setVertices(lua_State *L)
{
	...
	if (!lua_isnoneornil(L, 4))
	{
		...
		if (vertcount <= 0)
			return luaX_error(L, "Vertex count must be greater than 0.");
	}
	...
	if (vertstart >= totalverts || vertstart < 0)
		return luaX_error(L, "Invalid vertex start index (must be between 1 and %d)", totalverts);
	...
	if (luax_istype(L, 2, Data::type))
	{
		...
		if (vertstart + vertcount > totalverts)
			return luaX_error(L, "Too many vertices (expected at most %d, got %d)", totalverts - vertstart, vertcount);
...
int w_Mesh_setVertexAttribute(lua_State *L)
{
	...
	if (attribindex < 0 || attribindex >= (int) vertexformat.size())
		return luaX_error(L, "Invalid vertex attribute index: %d", attribindex + 1);
...
int w_Mesh_attachAttribute(lua_State *L)
{
	...
	else
	{
		...
		if (buffer == nullptr)
			return luaX_error(L, "Mesh does not have its own vertex buffer.");
	}
...
int w_Mesh_getAttachedAttributes(lua_State *L)
{
	...
	for (int i = 0; i < (int) attributes.size(); i++)
	{
		...
		if (!getConstant(attrib.step, stepstr))
			return luaX_error(L, "Invalid vertex attribute step.");
...
int w_Mesh_setVertexMap(lua_State *L)
{
	...
	if (luax_istype(L, 2, Data::type))
	{
		...
		if (indexcount < 1 || indexcount * datatypesize > d->getSize())
			return luaX_error(L, "Invalid index count: %d", indexcount);
...
int w_Mesh_getDrawMode(lua_State *L)
{
	...
	if (!getConstant(mode, str))
		return luaX_error(L, "Unknown mesh draw mode.");
