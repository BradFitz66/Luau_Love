...
int w_Source_setPitch(lua_State *L)
{
	...
	if (p != p)
		return luaX_error(L, "Pitch cannot be NaN.");
	if (p > std::numeric_limits<lua_Number>::max() ||
			p <= 0.0f)
		return luaX_error(L, "Pitch has to be non-zero, positive, finite number.");
	t->setPitch(p);
	return 0;
}
...
int w_Source_seek(lua_State *L)
{
	...
	if (offset < 0)
		return luaX_argerror(L, 2, "can't seek to a negative position");
...
int w_Source_setVolumeLimits(lua_State *L)
{
	...
	if (vmin < .0f || vmin > 1.f || vmax < .0f || vmax > 1.f)
		return luaX_error(L, "Invalid volume limits: [%f:%f]. Must be in [0:1]", vmin, vmax);
	t->setMinVolume(vmin);
	t->setMaxVolume(vmax);
	return 0;
}
...
int w_Source_setAttenuationDistances(lua_State *L)
{
	...
	if (dref < .0f || dmax < .0f)
		return luaX_error(L, "Invalid distances: %f, %f. Must be > 0", dref, dmax);
	luax_catchexcept(L, [&]() {
...
int w_Source_setRolloff(lua_State *L)
{
	...
	if (rolloff < .0f)
		return luaX_error(L, "Invalid rolloff: %f. Must be > 0.", rolloff);
...
int w_Source_setAirAbsorption(lua_State *L)
{
	...
	if (factor < 0.0f)
		return luaX_error(L, "Invalid air absorption factor: %f. Must be > 0.", factor);
...
int setFilterReadFilter(lua_State *L, int idx, std::map<Filter::Parameter, float> &params)
{
	...
	if (lua_type(L, -1) == LUA_TNIL)
		return luaX_error(L, "Filter type not specificed.");
	...
	while (lua_next(L, idx))
	{
		...
		{
#define luax_effecterror(l,t) luaX_error(l,"Bad parameter type for %s %s: " t " expected, got %s", typestr, keystr, lua_typename(L, -1))
...
#undef luax_effecterror
			...
			luaX_error(L, "Invalid '%s' Effect parameter: %s", typestr, keystr);
...
int w_Source_queue(lua_State *L)
{
	...
	if (luax_istype(L, 2, love::sound::SoundData::type))
	{
		...
		if (offset < 0 || length > s->getSize() - offset)
			return luaX_error(L, "Data region out of bounds.");
...
int w_Source_getType(lua_State *L)
{
	...
	if (!Source::getConstant(type, str))
		return luaX_error(L, "Unknown Source type.");
