	...
	if (!luax_istype(L, 1, love::sound::SoundData::type))
	{
		if (!luax_istype(L, 1, love::sound::Decoder::type))
		{
			...
			if (stype == Source::TYPE_QUEUE)
				return luaX_error(L, "Cannot create queueable sources using newSource. Use newQueueableSource instead.");
		}
...
int w_setEffect(lua_State *L)
{
	...
	if (lua_type(L, -1) == LUA_TNIL)
		return luaX_error(L, "Effect type not specificed.");
...
extern "C" int luaopen_love_audio(lua_State *L)
{
	...
	if (instance == nullptr)
		return luaX_error(L, "Could not open any audio module.");
