...
int w_setIdentity(lua_State *L)
{
	...
	if (!instance()->setIdentity(arg, append))
		return luaX_error(L, "Could not set write directory.");
...
int w_setSource(lua_State *L)
{
	...
	if (!instance()->setSource(arg))
		return luaX_error(L, "Could not set source.");
...
FileData *luax_getfiledata(lua_State *L, int idx, bool ioerror, int &nresults)
{
	...
	if (!data && !file)
	{
		nresults = luaX_argerror(L, idx, "filename, File, or FileData expected");
	...
	else if (file && !data)
	{
		...
		catch (love::Exception &e)
		{
			...
			if (ioerror)
				...
				nresults = luaX_error(L, "%s", e.what());
...
Data *luax_getdata(lua_State *L, int idx)
{
	...
	if (!data && !file)
	{
		luaX_argerror(L, idx, "filename, File, or Data expected");
		return nullptr; // Never reached.
	}
...
int w_newFileData(lua_State *L)
{
	...
	if (luax_istype(L, 1, Data::type))
	{
		...
		return luaX_argerror(L, 1, "string or Data expected");
...
int w_getInfo(lua_State *L)
{
	...
	if (instance()->getInfo(filepath, info))
	{
		...
		if (!Filesystem::getConstant(info.type, typestr))
			return luaX_error(L, "Unknown file type.");
...
static int w_write_or_append(lua_State *L, File::Mode mode)
{
	...
	if (luax_istype(L, 2, love::Data::type))
	{
		...
		return luaX_argerror(L, 2, "string or Data expected");
...
int w_lines(lua_State *L)
{
	if (lua_isstring(L, 1))
	{
		...
		return luaX_argerror(L, 1, "expected filename.");
...
int w_load(lua_State *L)
{
	...
	if (loadMode == Filesystem::LOADMODE_ANY)
		status = luax_loadbufferx(L, (const char *)data->getData(), data->getSize(), ("@" + filename).c_str());
	else
	{
		// Unsupported
		data->release();
		return luaX_error(L, "only \"bt\" is supported on this Lua interpreter\n");
	}
#endif
	...
	{
	case LUA_ERRMEM:
		return luaX_error(L, "Memory allocation error: %s\n", lua_tostring(L, -1));
	case LUA_ERRSYNTAX:
		return luaX_error(L, "Syntax error: %s\n", lua_tostring(L, -1));
	default: // success
		return 1;
	}
}
