...
int w_newRandomGenerator(lua_State *L)
{
	...
	if (lua_gettop(L) > 0)
	{
		...
		if (should_error)
			return luaX_error(L, "%s", lua_tostring(L, -1));
	}
...
int w_triangulate(lua_State *L)
{
	...
	if (vertices.size() < 3)
		return luaX_error(L, "Need at least 3 vertices to triangulate (got %d).", (int)vertices.size());
...
extern "C" int luaopen_love_math(lua_State *L)
{
	...
	luax_loadbufferx(L, math_lua, sizeof(math_lua), "=[love \"wrap_Math.lua\"]");
	lua_pushvalue(L, -2);
