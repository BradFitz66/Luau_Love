...
int w_pump(lua_State *L)
{
	...
	luax_catchexcept(L, [&]() {
		//Ensure no null instance
		if (!instance())
			throw love::Exception("love.event module not initialized");
		
		instance()->pump(waitTimeout);
	});
	return 1;
}
...
int w_push(lua_State *L)
{
	...
	for (int i = 2; i <= nargs; i++)
	{
		...
		{
			vargs.clear();
			return luaX_error(L, "Argument %d can't be stored safely\nExpected boolean, number, string or userdata.", i);
		}
	}
...
static int drawCallbackInner(lua_State *L)
{
	...
	data->r->push(L); //Checked Referece->push, it doesn't appear to be pushing a null reference

	// Get the current function on the stack and check it's not nil
	// if(lua_isnil(L, -1))
	// 	throw love::Exception("Modal draw callback is nil");

	// lua_call(L, 0, 2); //This appears to attempt to call a nil value 
...
extern "C" int luaopen_love_event(lua_State *L)
{
	...
	if (luax_loadbufferx(L, (const char *)event_lua, sizeof(event_lua), "=[love \"wrap_Event.lua\"]") == 0)
		lua_call(L, 0, 0);
	else
		lua_error(L);
