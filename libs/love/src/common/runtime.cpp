...
static ObjectKey luax_computeloveobjectkey(lua_State *L, love::Object *object)
{
	...
	if ((key & (minalign - 1)) != 0)
	{
		luaX_error(L, "Cannot push love object to Lua: unexpected alignment "
...
static void luax_pushloveobjectkey(lua_State *L, ObjectKey key)
{
	...
	if (luax_isfulllightuserdatasupported(L))
		...
		luaX_error(L, "Cannot push love object to Lua: pointer value %p is too large", key);
	else
...
bool luax_checkboolflag(lua_State *L, int table_index, const char *key)
{
	...
	if (lua_type(L, -1) != LUA_TBOOLEAN)
	{
		...
		return luaX_argerror(L, table_index, err.c_str());
	}
	else
...
int luax_assert_argc(lua_State *L, int min)
{
	...
	if (argc < min)
		return luaX_error(L, "Incorrect number of arguments. Got [%d], expected at least [%d]", argc, min);
	return 0;
}
...
int luax_assert_argc(lua_State *L, int min, int max)
{
	...
	if (argc < min || argc > max)
		return luaX_error(L, "Incorrect number of arguments. Got [%d], expected [%d-%d]", argc, min, max);
	return 0;
}
...
int luax_assert_function(lua_State *L, int idx)
{
	if (!lua_isfunction(L, idx))
		return luaX_error(L, "Argument must be of type \"function\".");
	return 0;
}
...
int luax_assert_nilerror(lua_State *L, int idx)
{
	if (lua_isnoneornil(L, idx))
	{
		if (lua_isstring(L, idx + 1))
			return luaX_error(L, lua_tostring(L, idx + 1));
		else
			return luaX_error(L, "assertion failed!");
	}
	return 0;
}
...
int luax_register_searcher(lua_State *L, lua_CFunction f, int pos)
{
	...
	if (lua_isnil(L, -1))
		return luaX_error(L, "Can't register searcher: package table does not exist.");
	...
	if (lua_isnil(L, -1))
		return luaX_error(L, "Can't register searcher: package.loaders table does not exist.");
...
int luax_getfunction(lua_State *L, const char *mod, const char *fn)
{
	...
	if (lua_isnil(L, -1)) return luaX_error(L, "Could not find global love!");
	lua_getfield(L, -1, mod);
	if (lua_isnil(L, -1)) return luaX_error(L, "Could not find love.%s!", mod);
	lua_getfield(L, -1, fn);
	if (lua_isnil(L, -1)) return luaX_error(L, "Could not find love.%s.%s!", mod, fn);
...
int luax_insistregistry(lua_State *L, Registry r)
{
	...
	default:
		return luaX_error(L, "Attempted to use invalid registry.");
	}
}
...
extern "C" int luax_typerror(lua_State *L, int narg, const char *tname)
{
	...
	return luaX_argerror(L, narg, msg);
}

int luax_enumerror(lua_State *L, const char *enumName, const char *value)
{
	return luaX_error(L, "Invalid %s: %s", enumName, value);
}
...
int luax_enumerror(lua_State *L, const char *enumName, const std::vector<std::string> &values, const char *value)
{
	...
	return luaX_error(L, "Invalid %s '%s', expected one of: %s", enumName, value, valueString.c_str());
}
...
void luax_runwrapper(lua_State *L, const char *filedata, size_t datalen, const char *filename, const love::Type &type, void *ffifuncs)
{
	...
	if (lua_istable(L, -1))
	{
		...
		luax_loadbufferx(L, filedata, datalen, chunkname.c_str());
		lua_pushvalue(L, -2);
		if (ffifuncs != nullptr)
...
int luax_resume(lua_State* L, int n) {
  return lua_resume(L, NULL, n);
}

int luax_loadbufferx(lua_State* L, const char* buffer, size_t size, const char* name) {
